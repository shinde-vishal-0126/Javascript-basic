...
// A macro task (also called a task or macrotask) is any task that is scheduled to run in the task queue. Examples of macro tasks include:
...
// setTimeinterval()
// fetchApi()
// user intaraction like click event

// 2 Micro task 
// promices/callback
// async/await

// so this microtask and macrotask ultimately need to execute by javascript runtime 
// so microTask tack the precedence(or higher proprity)
...
// Event loop 
// Macrotask are part of the event loop are processed in main thread 
// They are responsible for keeping the application responsive and performing significant work 

// Example 
// Macro task loading a large dataset from a database , performing the complex data , rendering a complex user interface are example of macrotask 
...
// Micor task are basically used for  lightweight operation or callback
// Execution order 
// Microtask are typically executed immediately after the current macrotask has finished executing but before the next macro task start
...
// They are processed before the next macrotask in the queue 
// Example 
...
// Macrotask and micro task having the different type of Granularity su 
// Macrotask  represent larger and more time consuming task 
