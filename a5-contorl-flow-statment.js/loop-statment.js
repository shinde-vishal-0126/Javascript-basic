...
// while loop first check the condition then 
...
//? write a program to get the table of the any digits:
let no = 1
while(no<=10){
...
//? 2.# Do-while loop 
// 1. So do while loop is control flow statements execute at least  once and then check the condition after executing at least one (condition check at the end.)
...
// in do-while loop statement executed at least one and then check the condition 
// when you want to guarantee the execution of the loop body at lease once
...
if(isPrime){
    ...
    console.log('number is not primse number')
} 
...
// It is mainly used for iterating over object properties (keys) or the indices of an array.
//  It does not access the values directly but instead provides the key or index.
...
// It is specifically used for accessing the values of arrays, strings, or other iterable objects.
//  It provides a more natural way to iterate over elements of an array or collection without worrying about the index.
...
// for...of is best for iterables (like arrays, strings, Maps, Sets) when you need the actual value
