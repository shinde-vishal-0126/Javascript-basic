...
sharedVideo('subscribed').then((res)=>{
...
importantAction('shinde').then((res)=>{
...
// tTimeOut() function simulate an asynchronous operation
...
// Handling Dependent Results: In many cases, the result of one operation is required for the next operation. For example, you might need to fetch user data from an API and then use that data to fetch additional information from another API. Promise chaining allows you to pass the result of one promise to the next in the chain.

// Error Handling: In a promise chain, you can catch errors in one place rather than handling them individually for each asynchronous operation. The catch block at the end of the chain handles any errors that occur in any of the previous promises in the chain.
...
// In this example we have multiple handaller for one promise. This handler has no relationship. Also they execute independently and don't pass the result from one to another like the promise chain above.
