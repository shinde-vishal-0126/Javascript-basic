...
//  container are location in memory used to stored value : (javascript variable is just name of storage location)
...
//? What is differences between var, let, const in javascript ?

//? ==> 1. VAR :
//*  1. var having the "functional scope" / or may be Global scope
// 2. if you defined variable with var this variable available throughout the function in which they are defined.
// 3. Example :
...
// if try to access this firstName and lastName outside the function it show the//* References error:
...
// 7. we can also //* declared a variable again if it has been defined previously in the same scope;
...
// 10. it can be //* accessed before without initialization as default value is undefined:

// --------------------------------------------------------------------------------------

//? 2. LET :
// 1. let having //* "Block-scope:"
// 2. if you defined any variable with let so this variable only //* available block where they have defined.
...
function bioData() {
  ...
  console.log("full name :", name + lastName); // ReferenceError: lastName is not defined (let having block scope outside the block scope we are not able to access the lastName)
}
bioData();
...
function bioData() {
  let name = "vishal";
  // here we have to update or change the value of let variable like name (ot overwrite the previous value)
  // let name='akshu';
  name = "akshu";
  let age = 28;
  ...
  // console.log('full name :', name + lastName);// ReferenceError: lastName is not defined (let having block scope outside the block scope we are not able to aces the lastName)
}
bioData();
...
// 5. if you defined variable using the const you  can not updated or changes its value also you are not able to re-declared the variable in same scope;
// 6. Hoisting are not allowed
// 7. it can  not redeclared without initialization
// 8. it  can not be accessed before without initialization and it can not be declared without initialization.
//! NOTE :
...
// (i.e it can update or re-declare the  into the scope) (it allow redefined or declared)
...
test();
// 4. variable shadowing does not cross the boundary of scope

// 5. you have to shadow var variable with let variable but not vice-versa like let variable with var
// ex;
// e'let a = 'shinde
// function test(){
//     if(true){
...
// 6. if you try to shadow let variable with var it called illegal shadowing (it give the error variate already defined)
