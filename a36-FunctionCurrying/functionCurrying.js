...
// so bind() method return new function with specified this value and optionally pre-filled argument  but does not invoke immediately return new function are call in later
...
// Partial application reduces the arity of the original function by fixing some of its arguments, turning it into a function with a smaller arity.
...
// defined function and take function as an argument  and return curried function and take all the the argument that provided by a function whatever you have defined
function curry(fun){
    return function curriedFun(...args){ // using spread operator take the array of all of the arguments
        // next we have to check the argument length is Grater then or equal to the length of the function then return function with arguments
        // here in currying then number of nested function or curried function is equal to the number of argument pass to the function so here we have to check
        if(fun.length >= args.length){
            return fun(...args)

            // else we return function which will collect all of the next argument that are going to be return us spreading the value of the argument first and then the next argument calling the curried function recursively
            //
        }else{
